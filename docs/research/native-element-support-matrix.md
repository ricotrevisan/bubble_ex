...
| Text | plain literal or payload-resolved content; raw tag unset/`normal`, `h1`, `h2`, `h3`, or `h4` (`h1`/`h2`/`h3` occur in #28; explicit `normal` occurs in #35–#37; `h4` remains public-payload-only) | BBCode, mixed rich spans, automatic link/email recognition, workflow clicks, unobserved tag enums |
...
| **Text** *(frozen: literal paint/geometry; exporter semantics for `normal`/`h1`/`h2`/`h3`)* | Static/dynamic text; optional BBCode, recognized links/email, truncation, shrink/stretch height, optional SEO HTML tag.[B-VISUAL] | `Text` ×481; explicit `normal`, `h2`, `h4` tags | #28 freezes literal `h1`/`h2`/`h3` paint/geometry; #35–#37 freeze explicit `normal` paint/geometry and structurally pin `<p>`. Candidate tags are exporter design choices, not Bubble DOM parity; `h4` remains uncontrolled. | S1 preserves explicit observed `h1`/`h2`/`h3`/`h4`; unset/`normal` uses `<p>` or neutral `<div>/<span>` from block context. This is a planned exporter lowering, not Bubble DOM parity; never derive a heading level from font size.[HTML] | Typography, wrapping, line/letter/word spacing, alignment, ellipsis/intrinsic height. **S1 excludes BBCode, auto-linkification, mixed rich spans, and unobserved tag enums.** | **S1** |
...
| Plain Text and planned semantics | Literal `normal`/`h1`/`h2`/`h3` Text has frozen paint/geometry; exporter structure pins `<p>` and `<h1>`–`<h3>` | **Partial** — controlled `h4`, truncation, and exhaustive fit-height remain open |

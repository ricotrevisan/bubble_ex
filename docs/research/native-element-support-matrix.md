...
Adopt the matrix below as a **planning support matrix**, not as a claim that every listed variant has renderer parity. Bubble's official reference currently documents 28 native web element kinds plus reusable-element definition/instance machinery.[B-CATALOG] The first-party `bubble.io` payload proves a broader observed subset. The authorized issue #28 **partial parity fixture** now adds controlled source → DOM/CSS evidence for a narrower set: Page, Group in all four layout modes, plain Text paint/geometry, Shape, label-only Button, and an Email Input in its static empty/placeholder state.[C28] Image, Link, Text/Password Input, and the unexercised Email Input behaviors remain selected for S1 but are not parity-validated.
...
| Text | plain literal or payload-resolved content; raw tag unset/`normal`, `h1`, `h2`, `h3`, or `h4` (`h1`/`h2`/`h3` occur in the controlled source; `normal`/`h2`/`h4` occur in the public payload) | BBCode, mixed rich spans, automatic link/email recognition, workflow clicks, unobserved tag enums |
...
This evidence validates only the exercised variants and states. It does **not** validate Image, Link, Text/Password Input, Email Input behavior beyond its static empty/placeholder state, Button disabled/interaction states, Text truncation/fit-height/BBCode, page image backgrounds, or arbitrary style/condition precedence. For those rows and variants, official documentation and the public payload establish existence—not exact DOM/CSS/defaults or parity. A row or variant becomes parity-tested only after a frozen controlled case exercises it at behavior-derived viewports.[C24][R23][C28]
...
| **Text** *(controlled: literal paint/geometry; candidate semantics unvalidated)* | Static/dynamic text; optional BBCode, recognized links/email, truncation, shrink/stretch height, optional SEO HTML tag.[B-VISUAL] | `Text` ×481; explicit `normal`, `h2`, `h4` tags | #28 source: 20 literal nodes with explicit `h1`/`h2`/`h3` plus unset; static paint/geometry parity; candidate tags are design choices, not Bubble parity | S1 preserves explicit observed `h1`/`h2`/`h3`/`h4`; unset/`normal` uses `<p>` or neutral `<div>/<span>` from block context. This is a planned exporter lowering, not Bubble DOM parity; never derive a heading level from font size.[HTML] | Typography, wrapping, line/letter/word spacing, alignment, ellipsis/intrinsic height. **S1 excludes BBCode, auto-linkification, mixed rich spans, and unobserved tag enums.** | **S1** |
| **Image** | Static/dynamic source; Stretch, Rescale, Zoom, Adjust element height; alt text.[B-VISUAL] | `Image` ×89; `stretch`, `rescale`, `zoom` observed | None in issue #28 | `<img src alt>` for image content.[HTML] | CSS `object-fit`: `fill`, `contain`, `cover` respectively; Adjust-height uses intrinsic aspect ratio/auto height. Preserve crop, border/radius, opacity, failed/authenticated asset finding.[CSS-IMG] | **S1** |
...
| **Link** | Text or icon; explicit internal page/external URL, data/parameters, new tab, disabled, nofollow, wrapping.[B-VISUAL] | `Link` ×132 | None in issue #28 | Enabled, resolved destination → `<a href>` with correct `target`/`rel`. Literal disabled → non-navigating `<a>` without `href`, with destination retained as source metadata. Unresolved destination or disabled condition → non-navigating content plus canonical binding/finding; never guess activation.[HTML][C26] | Link typography/wrapping/focus/disabled presentation. **S1 is text-only**; do not convert Buttons to links.[I21] | **S1** |
...
| **Input** *(controlled: Email format/static state)* | Single-line formats: Text, Email, Password, Integer, Decimal, Address, US Phone, Percentage, Currency, Date, Euro date, Text (numbers only); required/disabled, initial value, mask/validation.[B-INPUT] | `Input` ×16; explicit `text`, `email`, `password`, `date` | #28: one Email Input in static empty/placeholder state; paint/geometry parity | `<input>` with matching native type only where semantics align.[HTML] | Placeholder/value, typography, border/background, focus/disabled/required. **S1 only Text (`type=text`), Email, Password**. Bubble masks, Google address validation, numeric/currency/date parsing are deferred. | **S1** |
...
| Plain Text and planned semantics | Literal text has exact selected typography/wrapping; candidate `h1`/`h2`/`h3` tags are unvalidated design choices | **Partial** — semantic lowering parity, `normal`/`h4` controlled cases, truncation, and exhaustive fit-height remain open |
| Image modes/assets | No Image in the controlled slice | **Open** |
...
| Text-only Link | No Link in the controlled slice | **Open** |
| Text/Email/Password Input | One static Email Input exercises native input geometry, typography, placeholder paint, and CTA wrapping | **Partial** — Text/Password and focus/invalid/required/disabled behavior remain open |
...
Until production code and the remaining cases exist, **S1 means selected for implementation**. “Controlled” means only that issue #28 proved the named fixture variant; it does not silently widen support.[C24][C28]
...
- **Controlled coverage is partial.** Image, Link, Text/Password Input behavior, Button interaction/disabled states, Text truncation/fit-height and controlled `normal`/`h4`, page image backgrounds, and many native kinds have no controlled case.[C28]

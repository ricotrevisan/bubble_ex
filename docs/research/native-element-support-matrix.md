...
| Floating Group | Native container with normalized content (WTF-407); `position: fixed` against top/bottom/both and left/right/center/both edges; `is_visible: false` starts hidden; `runtime` records placement and layer | `bpgwgmpz` (always-visible top-right) | Other anchors, reusable bases, parallax, and show/hide states lack parity evidence |
...
| Popup | Native container with normalized content (WTF-407), emitted with `hidden`; shown, it is fixed, horizontally centered, 100px from the top (or vertically centered); `runtime` records backdrop, Escape dismissal and layer | `bpndkqfs`, `bptvorpv` initial states; `bptvorpv` opened geometry against the committed source observation | The static export never opens it; backdrop, focus trapping and Escape dismissal are model data for a runtime adapter; vertical centering lacks a capture |
| Group Focus | Native container with normalized content (WTF-407), emitted with `hidden`; shown, CSS anchor positioning puts it below its reference plus offsets | `bptvorpv` initial state and opened geometry against the committed source observation | Outside-click dismissal and collision behavior are model data only; references outside the CSS scope (page or one reusable instance) fall back to the static position |
| Repeating Group | One horizontal show-all row from a bounded literal list; direct Image/Text/Shape/Icon templates; other variants remain placeholders whose cell template is normalized as `runtime` container content (not rendered) | `bpgwgmpz` empty boundary; literal lists have unit tests and landing-page observations | No searches, nested templates, pagination, animation, or virtualization; at most 100 items and 1,000 projected leaves |
...
| Table / Video / Map / Alert / other HTML / Built on Bubble | Unsupported placeholders; a placeholder container (Table, plugin container) keeps its normalized content as `runtime` container content, not rendered | No native parity case | No promotion based only on an approximate box |

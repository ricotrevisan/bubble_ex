> Historical planning research, retained for its observations and source hashes. For current implementation and evidence, use the [current support matrix](native-element-support-matrix.md).
...
Adopt the matrix below as a **planning support matrix**, not as a claim that every listed variant has renderer parity. Bubble's official reference currently documents 28 native web element kinds plus reusable-element definition/instance machinery.[B-CATALOG] The first-party `bubble.io` payload proves a broader observed subset. The authorized issue #28 **partial parity fixture** adds controlled source → DOM/CSS evidence for Page, Group in all four layout modes, plain Text paint/geometry, Shape, label-only Button, and an Email Input in its static empty/placeholder state.[C28] Frozen cases #35–#38 extend that narrow evidence to public Image modes, text-only Link, static Text/Password Input, and explicit normal/H4 Text. Issue #32 adds the first characterized S2 slice: fixed-height Multiline Input plus literal/static Checkbox, Dropdown, and Radio Buttons.[C32] Issue #42 adds a second controlled S2 slice:[C42] hydrated and recursively nested static Group-based reusables, a literal Font Awesome 4 star bundled from Bubble's SVG sprite, and an always-visible top-right Floating Group. Its empty Repeating Group remains an explicit dimension-preserving runtime boundary. Unexercised variants remain placeholders without a parity claim.
...
This evidence validates only the exercised variants and states. Frozen cases now cover public Image Stretch/Rescale/Zoom/Adjust-height (#35), text-only resolved/disabled Link (#36), static Text/Password Input alongside the #28 Email Input (#37), explicit normal/H4 Text (#38), and fixed-height Multiline Input plus literal/static Checkbox, Dropdown, and Radio Buttons (#32).[C32] It does **not** validate authenticated/dynamic Image, icon/dynamic Link, fit-height Multiline Input, dynamic checkbox status or choice controls, Input focus/invalid/required/disabled behavior, Button disabled/interaction states, Text truncation/fit-height/BBCode, page image backgrounds, or arbitrary style/condition precedence. A row or variant becomes parity-tested only after a frozen controlled case exercises it at behavior-derived viewports.[C24][R23][C28]
...
| **Repeating Group** *(controlled runtime boundary)* | Repeated cell/list/grid; fixed or dynamic rows/columns, vertical/horizontal/wrapped/reverse scroll, show-all, masonry, separators.[B-RG] | `RepeatingGroup` ×33 | #42 `[C42]`: empty/runtime-boundary box with authored cell-template metadata; source/export outer geometry and paint are pinned | Dimension-preserving generic grid placeholder plus an explicit `unsupported_element` finding; no list/table semantics are inferred.[HTML] | Cell expansion, data/pagination/loading, virtualization/scroll, masonry, and current-cell context remain unsupported until a deterministic data contract is captured. | **D** |
...
| **Floating Group** *(controlled: always-visible top-right)* | Attached to screen edge, remains while page scrolls; horizontal/vertical attachment and z-index.[B-FLOAT] | `FloatingGroup` ×1 | #42 `[C42]`: literal, always-visible top-right Column with fixed width, margin, paint, and nested Text at desktop/mobile | Generic `<div>` with `position: fixed`; captured top/right attachment metadata becomes zero-edge anchors and source margin supplies offsets. | Runtime visibility, parallax, non-static expressions, collision behavior, and other attachment modes need separate captures. | **S2** |
...
| **Icon** *(controlled: static Font Awesome 4)* | Material UI, Phosphor, Bootstrap, Ionic, Font Awesome 4/6 free, Feather; color and spin.[B-VISUAL] | native `Icon` ×127; plugin icon nodes also present | #42 `[C42]`: literal `fa fa-star`, local Bubble Font Awesome 4 SVG sprite, color, geometry, and desktop/mobile paint | Decorative static icon is emitted as an accessibility-hidden inline SVG `<use>` backed by a content-addressed local sprite. | Only literal `fa fa-*` metadata is lowered. Other libraries, animation, dynamic glyphs, and interactive accessible-name behavior remain placeholders. | **S2** |
...
| **Multiline Input** *(controlled: fixed height)* | Multi-line plain text, character limit, optional stretch to fit.[B-INPUT] | `MultiLineInput` ×11 | #32: one fixed-height literal value/placeholder/maxlength case; static parity.[C32] | `<textarea>`.[HTML] | Preserve value/placeholder/maxlength/disabled. Fit-height-to-content remains a placeholder until separately characterized. | **S2** |
| **Checkbox** *(controlled: literal checked/unchecked state)* | Labeled checkbox; checked/unchecked/dynamic initial state; optionally required checked.[B-INPUT] | none | #32: literal checked/required and unchecked/optional states with labels; static parity.[C32] | Labeled `<input type="checkbox">`.[HTML] | Preserve checked/required/disabled and label hit target; dynamic status remains a binding-backed placeholder. | **S2** |
| **Dropdown** *(controlled: static choices)* | Static or dynamic choices; Bubble explicitly says it uses the browser's native dropdown component.[B-INPUT] | `Dropdown` ×7; all explicitly dynamic | #32: literal newline-delimited choices, placeholder, default, and mandatory flag; static parity.[C32] | Labeled `<select>`/`<option>`.[HTML] | Preserve static choices/default/placeholder and disabled/required state. Dynamic choices remain a binding-backed placeholder. | **S2** |
...
| **Radio Buttons** *(controlled: static choices)* | Static/dynamic exclusive choices; configurable columns and control color.[B-INPUT] | no native discriminator observed; similarly named reusable instances exist | #32: literal newline-delimited choices, default, mandatory flag, and one-column primary controls; static parity.[C32] | Group of labeled `<input type="radio">`; group label is required when source provides one.[HTML] | Preserve static options, selected/default/required/disabled, and characterized control paint. Dynamic choices/captions remain a placeholder. | **S2** |
...
| **Reusable definition / instance** *(controlled: nested static Group roots)* | Definition base type is Group, Popup, or Floating Group; instances share definition but have parameters, states, and workflows.[B-REUSE] | `CustomDefinition` ×36; `CustomElement` ×54 | #42 `[C42]`: two outer instances of a painted Column definition, each recursively expanding a Row badge definition and icon | Normalize each definition once; recursively expand static children with stable instance-path ID prefixes; project captured root layout/paint onto each instance wrapper. Missing references and cycles stop expansion and emit findings. | Parameters, states, workflows, Popup/Floating reusable bases, runtime show/hide, and dynamic content remain bindings or placeholders. | **S2** |
...
- **Icon resources are only narrowly resolved.** Issue #42 captures literal Font Awesome 4 `fa fa-*` metadata and Bubble's local SVG sprite. The other documented libraries, dynamic glyphs, animation, fallback metrics, licensing checks, and interactive accessible-name sources remain uncaptured.[B-VISUAL]

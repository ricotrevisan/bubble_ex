defmodule BubbleEx.Frontend.FidelityTest do
  ...
  describe "load_case/1" do
    ...
    test "loads the frozen bpiordvb icon Button pin" do
      assert {:ok, case_} = Fidelity.load_case("bpiordvb")
    ...
    test "loads the frozen bpuzekut fit-height MultiLineInput pin" do
      assert {:ok, case_} = Fidelity.load_case("bpuzekut")
      assert case_.id == "bpuzekut"
      assert case_.source.page_path == "bubbleex-i55-fit-height-multiline"
      assert case_.semantics["textareas"] == ["bpwhiple"]
    end

    test "loads the frozen bpqkcldq date/integer Input pin" do
      assert {:ok, case_} = Fidelity.load_case("bpqkcldq")
      assert case_.id == "bpqkcldq"
      assert case_.source.page_path == "bubbleex-i57-date-integer-input"
      assert case_.semantics["inputs"] == ["bpouecpw", "bpyaqmrs"]
    end

    test "loads the frozen bpjehwxg extra Input formats pin" do
      assert {:ok, case_} = Fidelity.load_case("bpjehwxg")
      assert case_.id == "bpjehwxg"
      assert case_.source.page_path == "bubbleex-i59-input-formats"

      assert case_.semantics["inputs"] == [
               "bpsmmjkn",
               "bppctiid",
               "bpijcres",
               "bpphniid",
    ...
    test "loads the frozen bpoyzixi datetime/numbers/file pin" do
      assert {:ok, case_} = Fidelity.load_case("bpoyzixi")
      assert case_.id == "bpoyzixi"
      assert case_.source.page_path == "bubbleex-i63-datetime-numbers-file"
      assert case_.semantics["inputs"] == ["bperdgvn", "bpeqiyev", "bpfileiid"]
    end

    test "loads the frozen bpdimzwm PictureInput pin" do
      assert {:ok, case_} = Fidelity.load_case("bpdimzwm")
      assert case_.id == "bpdimzwm"
      assert case_.source.page_path == "bubbleex-i65-picture-input"
      assert case_.semantics["inputs"] == ["bpbyjqwg"]
    end

    test "loads the frozen bplvejcw slider/search pin" do
      assert {:ok, case_} = Fidelity.load_case("bplvejcw")
      assert case_.id == "bplvejcw"
      assert case_.source.page_path == "bubbleex-i67-slider-search"
      assert case_.semantics["inputs"] == ["bpilwdan", "bpsrchiid"]
    end

    test "loads the frozen bpndkqfs range/popup pin" do
    ...
    test "loads the frozen bpwipyqn BBCode Text pin" do
      assert {:ok, case_} = Fidelity.load_case("bpwipyqn")
      assert case_.id == "bpwipyqn"
      assert case_.browser.playwright == "1.55.1"
      assert case_.browser.chromium == "140.0.7339.186"
      assert case_.source.page_id == "bpwipyqn"
      assert case_.source.page_path == "bubbleex-i44-bbcode-text"
      assert case_.viewports == [390, 1440]
      assert case_.semantics["paragraphs"] == ["bpwipyqo"]
      assert case_.node_ids == ["bpwipyqn", "bpwipyqo", "bpwipyqp"]
    end
  ...
  describe "run/2" do
    ...
    @tag :fidelity
    @tag :tmp_dir
    test "bpuzekut passes the committed-reference gate", %{tmp_dir: tmp} do
      assert {:ok, report} = Fidelity.run("bpuzekut", out_dir: Path.join(tmp, "pkg"))
      assert report["status"] == "pass"
      assert report["mismatches"] == []
    end
    ...
    @tag :fidelity
    @tag :tmp_dir
    test "bpqkcldq passes the committed-reference gate", %{tmp_dir: tmp} do
      assert {:ok, report} = Fidelity.run("bpqkcldq", out_dir: Path.join(tmp, "pkg"))
      assert report["status"] == "pass"
      assert report["mismatches"] == []
    end
    ...
    @tag :fidelity
    @tag :tmp_dir
    test "bpoyzixi passes the committed-reference gate", %{tmp_dir: tmp} do
      assert {:ok, report} = Fidelity.run("bpoyzixi", out_dir: Path.join(tmp, "pkg"))
      assert report["status"] == "pass"
      assert report["mismatches"] == []
    end
    ...
    @tag :fidelity
    @tag :tmp_dir
    test "bpdimzwm passes the committed-reference gate", %{tmp_dir: tmp} do
      assert {:ok, report} = Fidelity.run("bpdimzwm", out_dir: Path.join(tmp, "pkg"))
      assert report["status"] == "pass"
      assert report["mismatches"] == []
    end
    ...
    @tag :fidelity
    @tag :tmp_dir
    test "bplvejcw passes the committed-reference gate", %{tmp_dir: tmp} do
      assert {:ok, report} = Fidelity.run("bplvejcw", out_dir: Path.join(tmp, "pkg"))
      assert report["status"] == "pass"
      assert report["mismatches"] == []
    end
    ...
    @tag :fidelity
    @tag :tmp_dir
    test "bpndkqfs passes the committed-reference gate", %{tmp_dir: tmp} do
      assert {:ok, report} = Fidelity.run("bpndkqfs", out_dir: Path.join(tmp, "pkg"))
      assert report["status"] == "pass"
      assert report["mismatches"] == []
    end

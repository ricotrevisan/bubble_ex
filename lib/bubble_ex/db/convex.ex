defmodule BubbleEx.Db.Convex do
  ...
  The emit is intentionally honest about what the Bubble IR can and cannot carry:
    ...
    * `:enum` columns become `v.string()` holding the option's stable key
      (`db_value`); the member values (in the Reader's `table.values`) are not
      rendered as `v.union(v.literal(...))`.
  ...
  // NOTE: :enum columns -> v.string() (option-set member values are not rendered).
  ...
  defp field_comment(%{type: :enum}, _pk), do: "enum -> option set (member values not rendered)"

  ...
  try {
    for (const width of widths) {
      ...
      const audit = await page.evaluate(() => {
        const queryExactlyOne = (selector) => {
          const matches = document.querySelectorAll(selector);
          if (matches.length !== 1) throw new Error(`${selector}: expected once, found ${matches.length}`);
          return matches[0];
        };
        const inspectElementLayout = (selector) => {
          const element = queryExactlyOne(selector);
        ...
        return {
          ...
          hero: inspectElementLayout(".e-hero"),
          heroCopy: inspectElementLayout(".e-hero-copy"),
          heroVisual: inspectElementLayout(".e-hero-visual"),
          navLinks: inspectElementLayout(".e-nav-links"),
          featureRow: inspectElementLayout(".e-feature-row"),
          fixedCluster: inspectElementLayout(".e-fixed-cluster"),
          fixedChild: inspectElementLayout(".e-cluster-a"),
          semantics: {
            page: inspectElementLayout(".e-page-index").tag,
            title: inspectElementLayout(".e-hero-title").tag,
            link: inspectElementLayout(".e-link-platform").tag,
            button: inspectElementLayout(".e-primary-cta").tag,
            input: inspectElementLayout(".e-email").tag,
            shapeHidden: queryExactlyOne(".e-visual-halo").getAttribute("aria-hidden"),
          },
          sourceOrder: [...queryExactlyOne(".e-hero").children].map((element) => element.dataset.exporterId),
        };
      });
      ...
      if (wrapped) {
        ...
        check(withinTolerance(copyCenter, visualCenter, 2), `${width}: unwrapped hero columns are not center-aligned`);
      }

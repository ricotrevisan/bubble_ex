...
const scrollScript = require('./scroll.cjs');
const scrollHash = Buffer.from(hash(Buffer.from(scrollScript)),'hex').toString('base64');
...
function build(capture, modules) {
  ...
  const navigation = (url, owner) => {
    try { const u = new URL(url, owner.url || capture.url); if (['https:','http:','mailto:','tel:'].includes(u.protocol) && !u.username && !u.password) return u.href; }
    catch {}
    warn('invalid_navigation'); return null;
  };
  function restoreStylesheets(doc, owner) {
    const state = owner.state;
    if (!Array.isArray(state?.stylesheets)) return;
    const removeSheets = node => {
      node.childNodes = (node.childNodes || []).filter(child => child.tagName !== 'style' && !(child.tagName === 'link' && attr(child,'rel') === 'stylesheet'));
      node.childNodes.forEach(removeSheets);
    };
    removeSheets(doc);
    const head = doc.childNodes.find(n => n.tagName === 'html').childNodes.find(n => n.tagName === 'head');
    for (const sheet of state.stylesheets) {
      scan.push(sheet.css || '',sheet.href || '',sheet.base || '');
      const node = html.parseFragment(sheet.href ? '<link rel="stylesheet">' : '<style></style>').childNodes[0];
      if (sheet.href) set(node,'href',sheet.href);
      else {
        if (typeof sheet.css !== 'string') throw new Error('invalid_stylesheet');
        node.snapshotBase = typeof sheet.base === 'string' ? sheet.base : owner.url;
        node.childNodes = [{nodeName:'#text',value:sheet.css,parentNode:node}];
      }
      if (sheet.media) set(node,'media',sheet.media);
  ...
  function sanitize(root, owner) {
    root.childNodes = (root.childNodes || []).filter(node => {
      ...
      if (['a','area'].includes(tag) && attr(node,'data-bubbleex-invalid-href') !== undefined) {
        warn('invalid_navigation');
        node.attrs = node.attrs.filter(a => !['href','data-bubbleex-invalid-href'].includes(a.name));
      }
      ...
      // Local file assets require a non-opaque origin. Each child still has its
      // own restrictive CSP; only the fixed scroll initializer can execute.
      if (tag === 'iframe') set(node,'sandbox',refs.get(attr(node,'src'))?.state?.scroll ? 'allow-same-origin allow-scripts' : 'allow-same-origin');
      if (tag === 'template' && attr(node,'shadowmode')) {
        const shadowIndex = attr(node.parentNode,'data-bubbleex-shadow');
        const shadowCSS = owner.state?.shadowStyles?.[Number(shadowIndex)];
      ...
      if (tag === 'style') {
        ...
        const styleOwner = node.snapshotBase ? {...owner,url:node.snapshotBase} : owner;
        const safeCSS = rewriteCSS(source,styleOwner).replace(/<\/style/gi,match => '\\3c ' + match.slice(1));
        node.childNodes = [{nodeName:'#text',value:safeCSS,parentNode:node}];
      } else sanitize(node,owner);
      if (node.content) sanitize(node.content,owner);
      return true;
    });
  }
  parts[0].state = capture;
  // Resolve each annotated embed through MHTML's frame Content-ID; URLs alone
  // cannot distinguish two instances of the same embedded document.
  for (const part of parts.filter(part => part.mime === 'text/html')) {
    const bindFrames = node => {
      const index = attr(node,'data-bubbleex-frame'), frame = refs.get(attr(node,'src'));
      if (node.tagName === 'iframe' && index !== undefined && frame) frame.state = capture.frames?.[Number(index)];
      for (const child of node.childNodes || []) bindFrames(child);
      if (node.content) bindFrames(node.content);
    };
    bindFrames(html.parse(part.body.toString('utf8')));
  }
  for (const part of parts) {
    ...
    if (part.mime === 'text/html' || part.mime === 'image/svg+xml') {
      ...
      if (part.mime === 'text/html') restoreStylesheets(doc,part);
      sanitize(doc,part);
      if (part.mime === 'text/html') {
        ...
        const policyText = part.state?.scroll ? CSP.replace("script-src 'none'",`script-src 'sha256-${scrollHash}'`) : CSP;
        const policy = html.parseFragment(`<meta http-equiv="Content-Security-Policy" content="${policyText}">`).childNodes[0];
        head.childNodes.unshift(policy); policy.parentNode = head;
        if (part.state?.scroll) {
          const body = htmlNode.childNodes.find(n => n.tagName === 'body');
          const script = html.parseFragment(`<script>${scrollScript}</script>`).childNodes[0];
          body.childNodes.push(script); script.parentNode = body;
        }
      }

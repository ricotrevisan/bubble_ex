...
async function capture(url, options, modules, audit) {
  ...
  try {
    ...
    const state = await page.evaluate(prepare);
    ...
    const result = {schema:1,url:page.url(),viewport:options,browser:browser.version(),platform:process.platform,arch:process.arch,capturedAt:new Date().toISOString(),archive,resources,shadowStyles:state.shadowStyles,findings:[...state.findings,...errors.map(code => ({code}))]};
    if (audit) {

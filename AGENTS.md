...
- **BubbleEx.Tasks** - Works a plan in the owner's repository (WTF-375, `mix wtf.task next|show|claim|release|complete|review|note|audit|sync`, dev-only). **Advisory only**: the plan, manifest, task states, results, tests and code are all writable by the agent being verified, so every verdict is recorded and printed as `advisory` (trusted verification anchored outside the repo is WTF-411); `plan_sha256` is an integrity hash, not authentication. One canonical `.wtf/tasks/<task id, percent-encoded>.json` per task (`Tasks.State`: status open/done/needs_reverify/removed, time-limited claim, implementer labels, basis `source_sha256`, per-criterion evidence (binding, outcome, file refs + SHA-256, never command output), review pinned to source and reviewed evidence, notes, reverify, `mode: advisory`) via `Tasks.Store` (atomic exclusive-temp writes; `sync` writes states before the plan); readiness by blocking dependencies (`coordinate` edges only shown), claims and `needs_decision` notes; `complete` (`Tasks.Verifier`) binds every criterion and refuses on any failure (only `attested` waivable, with a reason); subtasks are re-checked in the same run and a failing subtask flips its parent; `independent_review` compares reviewer labels with implementer labels plus a git-author hint (`Tasks.Git`, spoofable, needs full history); every result naming a criterion must pass (no newer-pass masking; unused results are reported as ignored). Code checks are `Target.Phoenix.Checks` (`check_manifest`, `mix compile --warnings-as-errors`, format/credo, `data-bubble-id` attributes outside comments plus the surfaces' tagged render tests, `# bubble:step` comments (weaker), tests tagged `bubble: "<subject>"` read from the test AST and run once per subject by exit status, `Verify.Result` evidence through `Result.evaluate/3`, replay Bubble-verified). `scripts/phoenix_compile_check/task_cli.sh` runs complete/audit (including a tagged-test check) end to end on a generated project

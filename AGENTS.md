...
- **BubbleEx.Model** - Typed, stack-neutral model of an app's data (data types, fields with content types, built-in fields, option sets with stable keys, API Connector types with cycle cuts, privacy rules), keyed by Bubble IDs with no target-language names; built from app JSON (it owns API Connector groups/calls and type resolution, `Model.External.Resolver`; `Model.ConnectorReader` reads call names in both key forms, URL hosts and header/parameter names and `private` flags, never parameter values; each call's `request` is a leak-safe `Model.ConnectorRequest` template (WTF-374): URL path/query and body JSON structure with placeholders referencing parameters; default-deny literals: only API versions, dictionary path words, media types, booleans/null and empty text are kept, the rest `redacted`) and `Privacy`, which types rule conditions against the Model's pre-privacy schema. Nothing outside `lib/bubble_ex/model` and `lib/bubble_ex/privacy` reads data-model members (test-enforced by `Db.OneInterpretationTest`). Target stacks map from it
...
- **BubbleEx.Target.ApiClients** - Maps the Model's API Connector groups and calls to a `%Target.ApiClients.Spec{}` (WTF-374; plain data: one module per group, one function per call with method, URL/query/header/body templates, auth, arguments, env variable names for private values and redacted literals, response decoding, residue with reasons); `Target.Phoenix.render(project, api_clients: spec)` prints it (`Phoenix.ApiClients`, xref-tested off the Model) as generated Req clients (`<App>.ApiClients.*`), `Req.Test` request-shape tests per call and `.wtf/api_clients.json`. No secret value is ever emitted

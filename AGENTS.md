...
- **BubbleEx.Target.Ash** - Maps the Model to `%Target.Ash.Project{}` (plain structs describing Ash resources, attributes, relationships, enums, typed structs, diagnostics and the per-app name map; no Ash dependency). `Target.Ash.Naming` owns Ash names (WTF-339); `Target.Ash.Decisions` applies owner decisions from `Decision.applicable/2` (cut 1: `refine_number_type`, `derive_from_related` as calculations, `rename` overrides; cut 2: `derive_count` as a list length or a count aggregate, `text_to_reference` as a `belongs_to` with no foreign key, `derive_reverse_relationship` as a `has_many`, `add_indexes` hints applied by default as `custom_indexes` (btree, GIN, trigram with `pg_trgm`, full text), geographic ones deferred; `project.applied`/`deferred`/`extensions`). `Target.Ash.Source` only prints a Project (xref-tested: no dependency on the Model or Reader); `Target.Ash.MatrixTests` (WTF-383, V3) prints ExUnit privacy-matrix tests for a `privacy: :unverified` Project from a `Verify.Matrix` (or the decoded `.wtf/verification` files): the seed in the Ecto sandbox with Bubble IDs as primary keys, actors via the generated `load_actor/1`, keyed `:read`, `:search` and visible fields per scenario against the expected recording (a Bubble one when given, else the model's), writing observations that `MatrixTests.results/3` turns into `Verify.Result`s; `scripts/ash_compile_check.sh` compiles its output against pinned Ash and runs those tests
- **BubbleEx.Target.Phoenix** - Renders a `%Target.Ash.Project{}` (`privacy: :omit`) as the file map of a complete Phoenix 1.8 + Ash 3 app (WTF-369): pinned `mix.exs` (`deps/1`: `Target.Ash.Versions` + Phoenix, LiveView, AshPhoenix, AshAuthentication, Oban/AshOban, Tailwind v4), config with env secrets, repo, the Ash modules one per file (printed by `Target.Ash.Source` with its `extend`/`extra_resources` options), AshAuthentication magic link on the User (`:ci_string` email with a unique identity, token resource, owned `Accounts.UserAuthentication` Spark fragment, generated `Accounts.Resources` for owned code, Oban sender stub with the link encrypted and unique per email), production requires `MAILER_ADAPTER`/`MAILER_FROM`, router with `/api/1.1/wf/:name`, layouts (no daisyUI), a smoke test. Generated files (Ash layer, workflow API entry, `assets/css/bubble.css`, `.wtf/names.json`) are hashed in `.wtf/generated.json` (`Phoenix.Manifest`, with project/decisions/applied hashes and the bubble_ex version); the rest is owned (scaffold once). `check_manifest/3` detects hand edits and (with `previous:`) stale generated files. Templates are EEx files under `lib/bubble_ex/target/phoenix/templates/`; xref-tested to stay off the Model and the mapper; `scripts/phoenix_compile_check.sh` builds, migrates and tests every fixture's app
- **BubbleEx.Index** - Deterministic symbol and reference index keyed by stable Bubble IDs (reads/writes per field, privacy-rule references, workflow call graph with cycles, execution class, invocation modes); a disposable derived cache built from `Workflows`, `Expression` and the `Model` (data types, fields, option sets, API Connector groups, privacy rules; pass `model:` to build the Model once); marketplace plugins are `plugin:<id>` symbols (installed, version) with `:uses_plugin` edges from their elements, actions and events
- **BubbleEx.Findings** - Deterministic model-refinement analyzers over the index that emit `BubbleEx.Finding`s (kind and category registry in `Finding.Kinds`): evidence, a stack-neutral proposal, confidence, a stable ID, a `proposal_sha256` and a `basis_sha256` (content of its subject and evidence symbols, via `Index.subject_sha256/2`); separate from diagnostics. The `:plugin` kind (WTF-376) is one decision per marketplace plugin with options `drop` / `replace_native` (a `Plugins.Catalog` equivalent) / `rebuild`, over `Plugins.Inventory` (members, uses, state reads, surfaces, workflows)
- **BubbleEx.Plan** - Stack-neutral migration task graph (WTF-366) from the Model, Index, normalized Frontend and `Decision.applicable/2`: generator nodes, one task per surface and backend folder with workflows as auto-closing subtasks, cycles as one task (SCC), acceptance/data/replay/delivery/cutover tasks, typed dependencies (cycle-safe, deterministic order), abstract criteria (`Plan.Criteria`), per-task `source_sha256` over its subgraph (covered symbols' semantic digests with `Plan.Content` keyed HMAC digests of raw definitions, data-model values and API calls; residue; `decisions_sha256` incl. decision records' states; the content algorithm and key ID), a `symbols` table (128-bit) and counts-only `coverage`; `Plan.diff/2` (`Plan.Diff`, WTF-367) classifies tasks unchanged/changed/added/removed, diffs covered symbols by ID and propagates `needs_reverify` along non-ordering edges (not `generate`/`early`) and to parents, never touching owned code. Plugin tasks wait for the owner's plugin decision (`decision:plugin/<id>` while undecided); a `drop` closes the plugin task and removes its uses. `Plan.Residue` says what a generator cannot lower (uncompiled expressions, plugins, placeholders, unsupported actions/events, styles); canonical JSON is `.wtf/plan.json`, read back by `Plan.decode/1` (`Plan.Codec`: schema version, `plan_sha256`, closed task/edge/criterion vocabularies; byte-for-byte round trip)
- **BubbleEx.Tasks** - Works a plan in the owner's repository (WTF-375, `mix wtf.task next|show|claim|release|complete|review|note|audit|sync`, dev-only): one canonical `.wtf/tasks/<task id, percent-encoded>.json` per task (`Tasks.State`: status open/done/needs_reverify/removed, time-limited claim, implementers, basis `source_sha256`, per-criterion evidence (binding, outcome, file refs + SHA-256, never command output), review, notes, reverify) via `Tasks.Store`; readiness by blocking dependencies (`coordinate` edges only shown), claims and `needs_decision` notes; `complete` binds every criterion and refuses on any failure (only `attested` waivable, with a reason); `subtasks_done`, `independent_review` (reviewer and completer not among the implementers of `args.of`), `decision_recorded` (in effect in the plan) and `attested` are task state; code checks are `Target.Phoenix.Checks` (`check_manifest`, `mix compile --warnings-as-errors`, format/credo, `data-bubble-id`/`bubble:` markers, `# bubble:workflow`/`# bubble:step` comments, tests tagged `bubble: "<subject>"`, and `Verify.Result` evidence through `Result.evaluate/3`: replay must be Bubble-verified); `audit` re-runs done tasks' checks and flips failures to `needs_reverify`; `sync` applies `Plan.diff/2` to states and writes only `.wtf/`. `scripts/phoenix_compile_check/task_cli.sh` runs complete/audit end to end on a generated project. **Trust**: everything in the owner repo is writable by the agent being verified, so runs are `advisory` (recorded as such) unless `--trusted`/`key:`: then `.wtf/plan.json` and `.wtf/generated.json` must match `.wtf/plan.sig` (`Plan.sign/2`/`verify/2`, `Plan.Signature`, HMAC-SHA256 with `WTF_PLAN_SIGNING_KEY`, never in the repo; `plan_sha256` is only an integrity hash), only results with a valid `.sig` count (all must pass), reviewer independence comes from git authors of state-file commits (`Tasks.Git`), attestations/waivers/advisory bindings (source-only traceability, step-order comments) need such a review, subtasks are re-verified (`Tasks.Verifier`). Reviews pin source and reviewed evidence and are dropped on flip; writes are atomic (states before plan in `sync`)
- **BubbleEx.Decision** - Stack-neutral, append-only owner decision records (kinds `finding`, `rename`, `parity_exception`; a parity exception's params are `scope`, `checks` (the `Verify.Check` names it excuses), `bubble_behavior`, `chosen_behavior`) with a strict versioned JSON codec, the `modify` parameter whitelist per transform (`Decision.Params`; `replace_plugin` takes `option`, one of the finding's `options`, and plugin findings cannot be rejected), `resolve/3` (states active/stale/orphaned/superseded/expired, `Decision.Resolved`), `applicable/2` (`Decision.Applied`) and `decisions_sha256/1` (generation inputs only)
- **BubbleEx.Verify** - Stack-neutral verification formats (WTF-381, V1 of WTF-358): `Verify.Seed` (synthetic records and personas; emails must use reserved domains), `Verify.Scenario` (persona, subject Bubble IDs, ops and what each observes), `Verify.Recording` (observations with oracle `bubble` (a `wtfreplay…` branch of a Bubble app ID, checked by `Verify.Replay`) or `model`; masks; staleness hashes) and `Verify.Result` (per-check status with the status rules of `Verify.Check`: privacy/data/auth differences only through the owner's decisions, agent quarantine ≤ 7 days from the first quarantine for behaviour checks, no self-declared owner waivers, structural never; `decision` = key + `proposal_sha256`). A decoded result never counts by itself: `Result.evaluate/3` links its decision against `Decision.resolve/3` (author, relevance, hashes), requires `app:` and evidence for every non-structural check, never counts `skipped`, trusts only listed reviewers and the cited complete recording (or, for L4 data/auth, the export hash), and says whether it passes and is Bubble-verified (`model` never is). Finding decisions explain only the checks and diff ops their kind maps to (`Verify.Check.explains?/3`) and only diffs within their subject; parity exceptions excuse only their named `checks`. `Verify.Value` holds canonical WTF-338 values (tagged JSON, numbers as floats, dates as ms, `json` for JsonValue), `Verify.Mask` masks nondeterministic parts, `Verify.Staleness` compares pinned hashes (scenario, source, seed, recording, `decisions_sha256`). Strict versioned codecs, canonical JSON. `Verify.Interpreter` (WTF-382, V2) evaluates privacy rules (through the expression IR) over seed data for a persona (visible, visible fields, searchable), with every unverified Bubble semantic a named flag of `Interpreter.Assumptions` (incl. `defaults_applied_at_creation`: an omitted field reads as its Model default; `nil` is explicitly empty; matrix seeds write defaults out and keep explicit empties as `null`) (default: the compiler's fail-safe reading) and each verdict listing the flags it depends on; unsupported rules make verdicts `:unknown`. `Verify.Matrix` synthesizes personas, seeds (`Matrix.Personas`, `Matrix.Solver`), `privacy_read` scenarios and `model`-oracle recordings plus a coverage report: rules solved, rules observable (mutation coverage, `Matrix.Coverage`) and per-flag dependent checks (`result/4` compares a subject's observations with them). The interpreter shares the expression compiler with the Ash target, so its Ash cross-check does not test the compiler; the hand-authored expectation tables do. `Verify.Replay` (WTF-384, V4) is the Bubble replay driver: `Replay.Target` accepts only a Bubble app ID, a `wtfreplay…` branch name and the operator-supplied branch ID (`branch_id:`, never `live`/`test`; Bubble serves branches at `/version-<id>`), optionally an owner-confirmed custom `host:` (exact-host match, HTTPS, no redirects), and builds every URL (`https://<host>/version-<branch_id>/api/1.1/…`); `Replay.Client` goes through `BubbleEx.HTTP` with persona tokens, call and wall budgets and backoff (creates retried only on 429); its searches are always constrained to ledger IDs (empty = no call; unconstrained results abort), except the preflight's anonymous exposure probe (`anonymous_probe/3`: field names and counts only); no token is sent before `Client.verify/2` checks, tokenless, Bubble's `/meta` and the kit's marker workflow (exact branch name + operator `marker_nonce`). `Replay.Kit.preflight/4` fails closed: an exposed type that shows a logged-out caller more than `_id`/Created/Modified Date (`:exposed`), or IDs only without a `/meta` schema or proof (`:may_leak`), or no record without proof or `allow_unproven` (`:unproven`), and a seed that signs users up without a safely exposed User Data API (logged-out-only otherwise), refuse the run. The seeder clears explicit empties (`nil` seed fields) after creation, journaled; a refused clear makes dependent scenarios incomplete, and it calls no workflow but the replay kit's sign-up/login (`Replay.Kit`, `docs/replay-kit.md`; app workflows wait for V7's replay-safety classes); updates/deletes go only through `Replay.Ledger`, an fsynced journal with intents written before each create; `Replay.Cleanup` deletes confirmed entries, finds unconfirmed sign-ups by exact run email and `resume/2`s a dead run; `Replay.Recorder` needs the dry-run `plan_sha256` and a `:ledger_dir`, runs under `try` so cleanup always runs, records each scenario twice with `Replay.Differential` masks (privacy/data/auth: only differing field values are masked; any visibility difference is unstable), reports calibration flags (incl. `dangling_ref_is_empty` via delete-after-seed) and refuses output that fails `Replay.CredentialScan`. Tests use `test/support/verify/fake_bubble.ex`; never a real app.

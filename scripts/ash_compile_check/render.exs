...
# for every fixture with privacy rules (and the private export), the
# matrix (BubbleEx.Verify.Matrix, oracle model) is written as owner-repo
# files to matrix/<name>/.wtf/verification/ and its ExUnit module, generated
# from those files, to test/matrix/<name>_privacy_matrix_test.exs; `mix
# test` runs them in the scratch project's test environment and
# matrix_results.exs scores what they observed. matrix/index.json lists them.
matrix =
  if privacy == :unverified do
    ...
    # Generated by scripts/ash_compile_check/render.exs: start the repos of the
    # privacy-matrix tests (an owner app's supervision tree does this).
    ...
    for {namespace, name, app} <- fixture_apps ++ private_apps,
      ...
      app_id = if name == "private_app", do: "private-app", else: "fixture-app"
      {:ok, synthesized} = BubbleEx.Verify.Matrix.synthesize(model, app: app_id)
      files = BubbleEx.Verify.Matrix.files(synthesized)
      ...
      {:ok, plan} = BubbleEx.Target.Ash.MatrixTests.plan(files)
      ...
      IO.puts(
        "rendered #{name}'s privacy-matrix tests: #{out.counts["scenarios"]} scenarios, " <>
          "#{out.counts["ops"]} ops, #{out.counts["observations"]} expected observations " <>
          "over #{out.counts["records"]} seed records"
      )
...
File.write!(Path.join(dir, "matrix_index.json"), Jason.encode!(matrix, pretty: true))

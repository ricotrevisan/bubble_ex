...
const sites = [['mochary','https://beta.mocharymethod.com/'],['bubble','https://bubble.io/']];
(async () => {
  ...
  for (const [site,url] of sites) for (const width of [390,768,1440]) {
    ...
    if (result.reference !== result.repeatReference) throw Error('Unstable source screenshot');
